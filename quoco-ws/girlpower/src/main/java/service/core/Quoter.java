...
public class Quoter extends AbstractQuotationService{
    ...
    public static void main(String[] args) {
        try {
            String hostConfig = args.length > 0 ? "path=" + args[0] : "path=http://localhost:9002/quotation?wsdl";
            ...
            ServiceInfo serviceInfo = ServiceInfo.create("_http._tcp.local.", "sqs", Port.GIRL_POWER_PORT, hostConfig);
            jmdns.registerService(serviceInfo);

            System.out.println(String.format("GirlPower server running at %s >>", jmdns.getInetAddress().getHostAddress()));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
